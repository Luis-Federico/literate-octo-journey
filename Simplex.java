...
/*}*/  
 ...
 System.out.println("La varuiable "+B+" vale: "+solucion[porque][B]);
}
 porque=porque+1;
 ...
 /* AQUI HAY QUE HALLAR EL MÍNIMO DE LAS MATRICES SOLUCIONES*/
	       ...
	       minimo.min(solucion,combinatoria,rango1,coste);
...
{ 
    ...
    public static void  min(float solucion[][],int porq,int rango2, float coste[])
    {
        int porque;
        int o,i;
		...
		{
solution[i]=0;
for (porque=0;porque<=porq;porque++)
{



    for(o=0; o< porq-porque;o++)
    {
if(coste[i]<0)
{
	 a=-1;
...
int b1=1;
int b2=1;
if(solucion[porque][i]<0)
{
	b1=-1;
}
if(solucion[porque+o][i]<0)
{
	b2=-1;
}

if ((solucion[porque][i]*b1>0)&&(solucion[porque+o][i]*b2>0))
{

    if ((solucion[porque][i]*a<=solucion[porque+o][i]*a)&&(solucion[porque][i]>=0))
    {
solution[i]=solucion[porque][i];
}
}
    }
}
